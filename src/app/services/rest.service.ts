...
export class RestService {
  ...
  public addBook(body: Book): Observable<any> {
    let headers = new Headers({'Content-Type': 'application/json'});
    let options = new RequestOptions({ headers: headers });
    return this.http.post(this.apiUrl.concat('agregarLibro'), body, options);
  }

  public editBook(body: Book): Observable<any> {
    let headers = new Headers({'Content-Type': 'application/json'});
    let options = new RequestOptions({ headers: headers });
    return this.http.put(this.apiUrl.concat('editarLibro'), body, options);
  }
}
