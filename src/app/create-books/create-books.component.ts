...
export class CreateBooksComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.isbn != -1) {
      this.editing = true;
      this.myForm.controls['isbn'].disable();
      const url = `buscarLibroIsbn/${this.isbn}`;
      
      this.service.queryGet(url).subscribe(
        response => {
          if (response) {
            const book: Book = response.json();
            this.name = book.nombre;
            this.quantity = book.cantidadInventario;
            this.availableQuantity = book.cantidadDisponible;
  ...
  public editBook(book: Book): void {
    this.service.editBook(book).subscribe(
      response => {
        let result = response;
        if (result) {
          this.router.navigate(["/books"]);
          swal({
            title: this.translate.instant("alerts.success"),
            text: this.translate.instant("alerts.edited_book"),
            type: "success",
            showCancelButton: false,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
